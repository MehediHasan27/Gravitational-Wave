...
Two black holes spiral in on the hero background, collide, and send a
gravitational wave out through the page. Real DOM content — headline, body copy,
buttons, the email input — bends, blurs, and splits into colour channels as the
wavefront crosses it. The page shakes. The sound is synthesised from the same
chirp law that drives the orbit.
...
a registered custom property `--w` written each frame from its distance to the
epicentre. CSS turns that one number into translate + skew + scale + blur +
chromatic `text-shadow`/`box-shadow`. The headline is split into one span per
glyph, each its own wavefront sampler, so it ripples letter by letter.
...
`blur + saturate + contrast + hue-rotate`, masked to a ring by an animated
`radial-gradient`. It distorts everything behind it, live text included, with
zero repaint of that DOM. The faint grid on `<body>` exists specifically so this
ring has something to visibly kink.
...
Three wavefronts fire, not one — a primary plus two slower, weaker echoes. A
single pass crosses the viewport in ~0.7s and reads as a glitch rather than an
event.
...
- **OFF** kills every warp channel and hides all four overlays while leaving the
  page fully usable.
- Flash is single, short, and dimmed — well under 3 flashes/sec.
- Total event length is ~9.5s with a hard end.
...
```js
__gw.seek(6.4)    // jump the timeline to t seconds and keep playing
__gw.scrub(6.6)   // pin the clock there so you can inspect a still
__gw.play()       // release the pin
__gw.bh.state     // live shader uniforms — mutate freely
__gw.bh.setDpr(1) // force a resolution
__gw.S            // timeline state (trauma, flash, shockT, epicentre)
```

Timeline constants are at the top of `assets/main.js`; the wavefront array
(`WAVES`) is right below them. Shader look is tuned via the constants in
`diskField` / `armField` / `palette` in `assets/blackhole.js`.
